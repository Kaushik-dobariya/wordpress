<?php
/**
 * About Section: Certifications & Trust
 *
 * Consumes authentic statutory credentials and food safety accreditations
 * from the spicecraft_certification taxonomy.
 *
 * @package SpiceCraft
 */
...
$cert_sec = function_exists( 'spicecraft_get_about_section' )
	? spicecraft_get_about_section( 'certifications' )
	: array();
...
<section id="certifications-trust" class="sc-about-certs" aria-label="<?php echo esc_attr( ! empty( $heading ) ? $heading : __( 'Product Certifications & Accreditations', 'spicecraft' ) ); ?>">
	<div class="sc-container">
		...
		<div class="sc-certs-grid">
			<?php foreach ( $terms as $term ) :
				...
				?>
				<div class="sc-cert-badge">
					<?php if ( ! empty( $logo_id ) ) : ?>
						<div class="sc-cert-badge__logo-wrap">
					...
					<?php else : ?>
						<div class="sc-cert-badge__icon-wrap" aria-hidden="true">
							<span class="dashicons dashicons-awards"></span>
						</div>
					<?php endif; ?>

					<h3 class="sc-cert-badge__name">
					...
					<?php if ( ! empty( $desc ) ) : ?>
						<p class="sc-cert-badge__desc"><?php echo esc_html( wp_strip_all_tags( $desc ) ); ?></p>
					<?php endif; ?>
				</div>
			<?php endforeach; ?>
		</div>
		...
		<?php if ( ! empty( $cta_label ) && ! empty( $cta_url ) ) : ?>
			<div style="text-align: center; margin-top: var(--sc-space-8);">
